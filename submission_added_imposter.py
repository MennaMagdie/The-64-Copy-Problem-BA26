...
def classify(oracle, state_ids):
    ...
    for sid in state_ids:
        # Each oracle.measure() call consumes ONE copy and returns ONE sample,
        # so loop to gather SHOT_BUDGET copies (bounded by remaining budget).
        n_copies = SHOT_BUDGET
        if hasattr(oracle, "remaining"):
            n_copies = min(n_copies, oracle.remaining(sid))
        n_copies = max(1, n_copies)

        samples = []
        for _ in range(n_copies):
            if hasattr(oracle, "measure"):
                bits = oracle.measure(sid)
            elif hasattr(oracle, "sample"):
                bits = oracle.sample(sid)
            else:
                bits = oracle(sid)
            samples.append(np.asarray(bits).reshape(-1))

        bits = np.vstack(samples)
